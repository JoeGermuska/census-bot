...
export const AMBIGUOUS_METRICS = {
  ...
  // Race & ethnicity ambiguity is now auto-derived from the Census metadata
  // by scripts/derive-acs-alternatives.mjs (Rule A: cross-table leaf match).
  // The chip rendering happens via buildDerivedAltChipPayload in the fast path,
  // so no hand-written entries are needed here. Adding "asian"/"black"/etc.
  // back in would create duplicate chip rows.
};
