...
import { getTableChanges } from "./acsTableChanges";
...
export function buildNuanceBanners(structured) {
  ...
  // 5. Documented table redesign — when the cited table appears in our
  // hand-curated lib/acsTableChanges.js catalog and the redesign falls
  // within or before the current vintage, surface the published note so
  // historical comparisons get an authoritative caveat. (Trend pipelines
  // also surface this on the chart itself; this banner is for stat cards
  // and ensures the same caveat reaches single-stat answers.)
  if (structured.variableId) {
    const changes = getTableChanges(structured.variableId);
    if (changes && changes.length > 0 && Number.isFinite(year)) {
      // Only flag when the user is looking at a year at or after the most
      // recent redesign — otherwise the banner adds noise to pre-redesign
      // queries that don't span the boundary.
      const mostRecent = changes[changes.length - 1];
      if (year >= mostRecent.year) {
        banners.push(
          `Table ${tableIdOf(structured.variableId)} was redesigned in ${mostRecent.year}: ` +
          `${mostRecent.summary} Pre-${mostRecent.year} comparisons may not be apples-to-apples.`
        );
      }
    }
  }
