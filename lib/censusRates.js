 ...
 * options object `{ year, dataset, numeratorMOE, fetcher }`. Pass
 * `numeratorMOE` (the raw MOE that came back with the numerator) to also
 * get back a computed MOE for the derived rate. The denominator must come
 * from the same dataset as the numerator so the rate is internally
 * consistent.
 *
 * Pass `fetcher: (variableId) => Promise<number>` to inject a custom
 * value-by-id fetcher — used by the trend pipeline to share its in-memory
 * cache (10 years × shared denominator = many cache hits). When `fetcher`
 * is provided, MOE computation is skipped because the cached fetcher
 * returns numbers, not {value, moe} pairs.
 */
export async function computeRateIfNeeded(variableId, rawCount, geoParams, apiKey, opts) {
  ...
  const { year, dataset = "acs/acs5", numeratorMOE = null, fetcher = null } =
    typeof opts === "string" ? { year: opts } : (opts || {});

  const wantMOE = numeratorMOE != null && config.kind != null && fetcher == null;

  try {
    let denominator, denominatorMOE = null;
    if (fetcher) {
      const denomNum = await fetcher(config.denominator);
      denominator = Number(denomNum);
    } else if (wantMOE) {
...
// Display strings for the formulas applied by computeRateMOE. Kept here
// (next to the math) so the chat layer can cite "how MOE was calculated"
// without duplicating the formula string elsewhere.
const KIND_FORMULA = {
  proportion: "MOE_P = (1/X) · √(MOE_Y² − P² · MOE_X²)",
  ratio:      "MOE_R = (1/X) · √(MOE_Y² + R² · MOE_X²)",
};

/**
 * Methodology blurb for a derived-rate variable. Returns null for raw
 * estimates (those use the API's published companion MOE directly).
 *
 * @returns {{ kind: string, denominator: string, formula: string|null } | null}
 */
export function getRateMethodology(variableId) {
  const config = RATE_CONFIG[variableId];
  if (!config) return null;
  return {
    kind: config.kind,
    denominator: config.denominator,
    formula: KIND_FORMULA[config.kind] || null,
  };
}
